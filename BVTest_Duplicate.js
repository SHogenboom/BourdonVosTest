...
function stimulusPresentation (appendObject, sizeCirckel, verticalCirckles, horizontalCirckles) {
    ...
    // LOOP to fill screen with max amount of responsive figures
        for (z = 2; z < totalFigures; z++) {                    // start at 2 because first canvas was already created
...
function maxFigures () {
    ...
    // Calculate max number of pictures to fit the window
        ...
        var totalFigures = ((((Math.floor(winWidth / stimWidth)) -1) * ((Math.floor(winHeight / stimHeight))))) +1; //Math.floor rounds the outcome down to an integer. 
        // +1 because first img object gets hidden so need to display an extra
...
function terminationButton () {  
    // STORE VARIABLES FOR CALCULATIONS
        ...
        sessionStorage.setItem("responseArray", responseArray);                     // responses made
        sessionStorage.setItem("correctionArray", correctionArray);                    // corrections made
        sessionStorage.setItem("clickArray", clickArray);                                     // mouseclicks made
        sessionStorage.setItem("responseOrderArray", responseOrderArray);   // order in which responses were made
        sessionStorage.setItem("dotsArray", dotsArray);                                     // amount of dots in each figure
